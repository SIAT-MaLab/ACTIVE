...
print(">>> Environment setup complete. Ready to process vOTU & vFAM saturation...")

main_file <- "merged_phage_stats_taxonomy.tsv"
if (!file.exists(main_file)) stop("Error: Main table 'merged_phage_stats_taxonomy.tsv' not found.")

print("Loading main metadata table...")
...
if (!"vFAM" %in% names(input_df) || !"vOTU" %in% names(input_df)) {
  stop("Error: Main table must contain both 'vOTU' and 'vFAM' columns.")
}

stats_file_votu <- "stats_baseline_vOTU_EXACT.tsv"
if (!file.exists(stats_file_votu)) stop("Error: Exact vOTU stats file not found.")
stats_df_votu <- read.delim(stats_file_votu, header = TRUE, sep = "\t", check.names = FALSE, quote = "")

stats_file_vfam <- "stats_baseline_vFAM_EXACT.tsv"
if (!file.exists(stats_file_vfam)) stop("Error: Exact vFAM stats file not found.")
...
list_sig_votu <- stats_df_votu %>% filter(Significance_Type == "Significant") %>% pull(vOTU) %>% unique()
list_sig_vfam <- stats_df_vfam %>% filter(Significance_Type == "Significant") %>% pull(vFAM) %>% unique()
...
print(paste("Data preparation complete. Total valid Phyla detected:", n_distinct(processed_df$Phylum)))
...
print(paste("Calculating saturation curves for", length(target_phyla), "phyla..."))
...
print("Processing... (This may take a few minutes depending on dataset size)")
...
plot_saturation <- function(data, title_str, y_label_str, file_name) {
  ...
  ggsave(file.path(output_dir, file_name), p, width = 10, height = 7)
...
plot_saturation(df_votu_sig,    "Saturation Curve: Exact Significant vOTUs by Phylum",  "Normalized Cumulative vOTUs (%)", "Curve_3_Significant_vOTU.pdf")
...
plot_saturation(df_vfam_sig,    "Saturation Curve: Exact Significant vFAMs by Phylum",  "Normalized Cumulative vFAMs (%)", "Curve_6_Significant_vFAM.pdf")

print("All tasks completed! Plots have been exported to the output directory.")
