...
df_clean <- data %>%
  ...
  select(vOTU, Genus, Family, Phylum) %>%
  distinct() 
votu_counts <- df_clean %>%
  count(vOTU) %>%
  filter(n > 1) 

cross_genus_df <- df_clean %>%
  filter(vOTU %in% votu_counts$vOTU)

host_phage_mat <- table(cross_genus_df$Genus, cross_genus_df$vOTU)
...
write.csv(edge_list, "Network_Edges_Summary.csv", row.names = FALSE)
...
graph <- as_tbl_graph(edge_list, directed = FALSE) %>%
  activate(nodes) %>%
  left_join(node_info, by = c("name" = "Genus")) %>%
  mutate(degree = centrality_degree(weights = Weight)) %>%
  filter(!node_is_isolated())
pdf("1t_Network_Topology.pdf", width = 12, height = 10)
set.seed(123) 
...
p <- ggraph(graph, layout = "graphopt", charge = 0.05, spring.length = 100) + 
  ...
  labs(title = "Host Genus Connectivity Network",
       subtitle = paste0("Nodes: ", length(unique(c(edge_list$From, edge_list$To))), 
                         " Genera | Edges: ", nrow(edge_list), " Connections"),
